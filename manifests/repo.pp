    ...
    'Debian': {
      case $facts['os']['name'] {
        'Debian': {
          ...
          if $varnish::version_major == '6' and versioncmp($facts['os']['distro']['release']['full'],'8.0') == -1 {
            fail('Varnish 6.0 and above is only supported on Debian 9 (Stretch) and above')
          }
        }

        'Ubuntu': {
          if $ver == '30' and versioncmp($facts['os']['release']['major'],'16.04') >= 0 {
          ...
          if $varnish::version_major == '6' and versioncmp($facts['os']['release']['major'],'16.04') == -1 {
