...
class varnish::params {
  case $facts['os']['family'] {
    ...
    'Debian': {
      if versioncmp($varnish::version_full,'6.1') >= 0 and versioncmp($facts['os']['distro']['release']['full'],'10.0') >= 0 {
      ...
      if versioncmp($facts['os']['distro']['release']['full'],$systemd_version) >= 0 {
        $os_service_provider = 'systemd'
      } else {
        $os_service_provider = 'sysvinit'
      }
